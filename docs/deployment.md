...
2. **VirtualService (Mirroring Configuration)**  
   The backend VirtualService is configured to mirror requests:
   - A copy of the request is sent to the shadow subset

3. **Mirrored Traffic Execution**  
   The mirrored request is processed by the shadow backend pods:
   - The shadow instance performs the full computation (e.g., loading the new model, running predictions).
   - Its response is discarded by the Istio sidecar proxy and never returned to the Frontend.

4. **Isolation from User Response**  
   The Frontend receives a response only from the primary backend. The mirroring process happens out-of-band, meaning the shadow launch has no impact on the correctness of the response sent to the user.
